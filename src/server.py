...
from src.utils.pydantic_models import RequestBody, ChatResponse
...
tool_permission_requests: Dict[str, str] = {}

func_agent_workflow = FuncAgentWorkflow(
    tools=tools,
...
async def chat_with_bot(request: RequestBody):
    ...
    try:
        ...
        handler: WorkflowHandler = await func_agent_workflow.run(
            model="gpt-4o-mini",
        ...
        if (
            tool_permission_requests[request.session_id]
            == request.input.approved_tool_call_id
        ):
            await handler.ctx.send_event(HumanResponseEvent(response=request.input.query))

        async for event in handler.stream_events():
            logger.info(f"Received event: {event}")
            if isinstance(event, InputRequiredEvent):
                logger.info(f"Received InputRequiredEvent: {event}")
                tool_permission_requests[request.session_id] = event.tool_call_id
                return JSONResponse(
    ...
    for message in chat_history:
        ...
        if text_blocks:  # check if the list is not empty
            ...
            formatted_history.append(
                {"role": message.role.value, "message": text_content}
            )
...
async def get_sessions(user_id: str, session_id: str):
    try:
        ...
        logger.error(
            f"Error getting messages from session {session_id} for user {user_id}: {str(e)}"
        )
        return []
